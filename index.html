...
<body>
    ...
    class EntanglementField {
        ...
        updateGeometryConstants() {
            ...
            // Mobile: Smaller triangles, tighter spacing
            // Desktop: Larger, more spread out
        ...
        initSystem() {
            ...
            for (let i = 0; i < rows; i++) {
                for (let j = 0; j < cols; j++) {
                    ...
                    for (let f = 0; f < 4; f++) this.tetraFaceStates[idx * 4 + f] = 0.3 + Math.random() * 0.7;
                    idx++;
                }
            }
            ...
            for (let i = 0; i < this.tetraCount; i++) {
                for (let j = i + 1; j < this.tetraCount; j++) {
                    ...
                    if ((dx * dx + dy * dy) < (distLimit * distLimit)) lines.push([i, j]);
                }
            }
        ...
        updateFluctuation() {
            ...
            for (let i = 0; i < this.lineCount; i++) {
                if (this.lineIsFluctuation[i] === 0 && this.lineStrength[i] < 0.01 && Math.random() < flickerChance) {
            ...
            for (let i = 0; i < this.tetraCount; i++) {
                const targetEnt = this.tetraConnectionCount[i] > 0 ? this.tetraConnectionCount[i] * 0.18 : 0;
        ...
        updatePhase() {
            ...
            if (this.phaseDisplay) {
                ...
                if (this.phaseDisplay.textContent !== text) this.phaseDisplay.textContent = text;
            }
            ...
            switch (this.systemPhase) {
                ...
                case 'pure':
                    ...
                    break;
                case 'fluctuating':
                    if (this.fluctuationEndTime === 0) this.fluctuationEndTime = this.globalTime + 5000 + Math.random() * 5000;
                    this.updateFluctuation();
                    return;
                case 'spreading':
                    for (let i = 0; i < this.lineCount; i++) {
                        if (this.lineActivationTime[i] === Infinity || this.globalTime < this.lineActivationTime[i]) this.lineStrength[i] *= 0.96;
                    }
                    for (let i = 0; i < this.tetraCount; i++) {
                        if (this.globalTime > this.tetraActivationTime[i]) this.tetraEntanglement[i] += (1 - this.tetraEntanglement[i]) * 0.005;
                    }
                    for (let i = 0; i < this.lineCount; i++) {
                        if (this.globalTime > this.lineActivationTime[i]) this.lineStrength[i] += (1 - this.lineStrength[i]) * 0.005;
                    }
                    break;
                case 'dissolving':
                    let maxLine = 0;
                    ...
                    for (let i = 0; i < this.tetraCount; i++) this.tetraEntanglement[i] *= 0.96;
            ...
            if (phase && phase.duration !== Infinity && elapsed > phase.duration) {
                ...
                this.phaseStartTime = this.globalTime;
                if (this.systemPhase === 'forming') this.initSystem();
            }
        }
        ...
        render() {
            ...
            for (let i = 0; i < this.lineCount; i++) {
                ...
                const isTrue = this.lineActivationTime[i] !== Infinity && this.globalTime > this.lineActivationTime[i];
            ...
            for (let i = 0; i < this.tetraCount; i++) {
                ...
                const r = 20 + ent * 54, g = 45 + ent * 79, b = 35 + ent * 54;
                ...
                const faceBaseAlpha = op * (0.55 + ent * 0.45), faceSize = 2 + ent * 2;
                for (let f = 0; f < 4; f++) {
            ...
            if (this.systemPhase === 'spreading' && this.seedX !== undefined) {
                const elapsed = this.globalTime - this.phaseStartTime;
                const radius = elapsed * 0.05, alpha = Math.max(0, 0.4 - radius * 0.00018);
                if (alpha > 0.01) {
                    this.ctx.beginPath();
        ...
        start() {
            if (!this.animationFrameId) this.animate(performance.now());
        }
    }
