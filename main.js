...
(function () {
  ...
  function updateFluctuation() {
    ...
    for (let i = 0; i < tetraCount; i++) {
      const targetEnt =
        tetraConnectionCount[i] > 0 ? tetraConnectionCount[i] * 0.18 : 0;
    ...
    switch (systemPhase) {
      ...
      case 'spreading':
        ...
        for (let i = 0; i < lineCount; i++) {
          if (
            lineActivationTime[i] === Infinity ||
            globalTime < lineActivationTime[i]
          ) {
            lineStrength[i] *= 0.97;
          }
        }
      ...
      case 'dissolving':
        ...
        // Gradually fade all entanglement links; keep the condensate geometry + opacity
        let maxLine = 0;
        for (let i = 0; i < lineCount; i++) {
        ...
        // Relax entanglement back toward the base condensate state
        for (let i = 0; i < tetraCount; i++) {
          tetraEntanglement[i] *= 0.96;
        }

        // Once links are basically gone and we've been dissolving for a bit,
        // return to the fluctuation phase (pure condensate + quantum flicker).
  ...
  function render() {
    ...
    for (let i = 0; i < tetraCount; i++) {
      ...
      for (let f = 0; f < 4; f++) {
        ...
        ctx.fillStyle = `rgba(201, 169, 98, ${
          faceBaseAlpha * brightness * 0.3
        })`;
        ctx.fill();
